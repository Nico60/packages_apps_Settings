...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("spirit_lock")) {
            resId = R.string.lockscreen_lock_icon_spirit;
    ...
    private void resizeSpiritLock() {
        Bitmap spiritLock = BitmapFactory.decodeResource(getResources(), R.drawable.spirit_lock);
        if (spiritLock != null) {
            String path = null;
            int px = requestImageSize();
            spiritLock = Bitmap.createScaledBitmap(spiritLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "spirit_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                spiritLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
